...
function getSumBetweenNumbers(n1, n2) {
  ...
  arr.fill(0);
  const narr = arr.map((item, index) => n1 + index);
  const res = narr.reduce((sum, current) => sum + current, 0);
  return res;
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.left >= right2 || rect1.top >= bottom2
    || right1 <= rect2.left || bottom1 <= rect2.top) {
    return false;
  }
  return true;
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < rowsA; i += 1) resC[i] = [];
